...
const getProyectosEspeciales = async(req,res)=>{
  try {
    const excel = XLSX.readFile('src/public/uploads/ProyectoseIndicadores.xlsx');
    var nombreHoja = excel.SheetNames;
    var datos = XLSX.utils.sheet_to_json(excel.Sheets[nombreHoja[0]]);
    //console.log(datos)
...
const getIndicadoresEspeciales = async (req, res)=>{
  try {
    const excel = XLSX.readFile('src/public/uploads/ProyectoseIndicadores.xlsx');
    var nombreHoja = excel.SheetNames;
    var datos = XLSX.utils.sheet_to_json(excel.Sheets[nombreHoja[1]]);
    ...
    for (let index = 0; index < datos.length; index++) {
      ...
      where indicativo.tbl_indicador.cod_indicador=$1
        `, [datos[index].codIndicador]);
        indicadorespecial.push(response.rows)
    }
    // console.log(estadosgtoespecial)
    res.status(200). json({
      Autor:'Alcaldía de Medellin - Departamento Administrativo de Planeación ',
    ...
    console.error('Error getIndicadoresEspeciales: ', error);
  }
}
