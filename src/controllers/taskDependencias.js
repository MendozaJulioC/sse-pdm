...
const getDependencias = async(req, res)=>{
    try {
        const response = await pool.query(`select * from dependencias.tbl_dependencias where cod_dep>700 and cod_dep<> 908 and cod_dep <>800 and cod_dep <958 and cod_dep <>901 and cod_dep<>957  and cod_dep <>954  order by cod_dep`);
        res.status(200). json({
...
const getAvanceDepPDM = async (req, res)=>{
    try {
        ...
        const response = await pool.query(`select * from indicativo.view_avance_dep where cod_responsable_reporte=$1`,[dependencia])
...
const getValStatDep= async(req, res)=>{
    try {
        const dependencia = req.params.cod_dependencia;
        const response = await pool.query(`select * from plan_accion.sp_dep_valstat($1)`, [dependencia]);
        res.status(200). json({
            Autor:'Alcaldía de Medellin - Departamento Administrativo de Planeación ',
            Fecha_Emision:'2020-08-30',
            Fecha_Inicial:'2020-01-31',
...
try {
    const dependencia = req.params.cod_dependencia;
    const response = await pool.query(`select  sum(pesoxavnt)as avancepond, sum(prog2022) as programado from indicativo.tbl_indicador where cod_responsable_reporte=$1`,[dependencia])
    res.status(200). json({
        Autor:'Alcaldía de Medellin - Departamento Administrativo de Planeación ',
        Version: '1.0',
        Cobertura:'Municipio de Medelín',
