...
const getEmail =  async (req, res)=> {
    try {
        ...
        const response = await aws_pool.query(`select * from auth.tbl_users where email=$1`, [email]);
        res.status(200).json({
...
const postRegisterUser = async(req, res)=>{
    try {
        ...
        const results = await aws_pool.query(`
...
const getIdLoguin =  async (req, res)=> {
    try {
        ...
        const response = await aws_pool.query(`select id, nom_usuario,email, tel_contacto, cod_dependencia, id_cargo from auth.tbl_users where id=$1 `, [id]);
        res.status(200).json({
            Autor:'Alcaldía de Medellin - Departamento Administrativo de Planeación ',
            Fecha_Emision:'2020-08-30',
            Fecha_Inicial:'2020-01-31',
